...
class Email:
    ...
    async def sendEmail(self) -> bool:
        ...
        from_name = "智慧眼科诊断系统"  # 发件人名称（可以是中文）
        from_email = settings.MAIL_USER  # 发件人邮箱地址
        if all(ord(c) < 128 for c in from_name):  # 如果昵称是纯 ASCII 字符
        ...
        message["Subject"] = Header("Python SMTP 测试邮件", "utf-8")
